...
import QuestWebAbi from "./QuestWeb.json";
...
export const reviewAction: Action = {
    name: "review",
    description: "Review a task submission on a QuestWeb",
    handler: async (
        runtime: IAgentRuntime,
        message: Memory,
        state: State,
    ...
    validate: async (runtime: IAgentRuntime): Promise<boolean> => {
        const privateKey = runtime.getSetting("QuestWeb_PRIVATE_KEY");
        const walletProvider = new WalletProvider(runtime);
        const contractAddress = runtime.getSetting(
            "QuestWeb_ADDRESS"
        ) as `0x${string}`;
