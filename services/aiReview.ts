            ...
            if (proofData.contract) {
              ...
              switch (network) {
                ...
                case 'opBNB Testnet':
                  ...
                  break;
                case 'Telos':
                  apiUrl = 'https://verified-evm-contracts.s3.amazonaws.com';
                  break;
                case 'Telos Testnet':
                  apiUrl = 'https://verified-evm-contracts-testnet.s3.amazonaws.com';
                  break;
              }
              ...
              } else if (network.startsWith('Telos')) {
                // Telos API call
                try {
                  const response = await fetch(
                    `${apiUrl}/${proofData.contract}/source.json`
                  );
                  ...
                  if (data.status === 'full' && data.files) {
                    content = data.files
                      .filter((file: any) =>
                        // 只保留 .sol 文件，过滤掉其他所有文件
                        file.name.toLowerCase().endsWith('.sol') &&
                        // 继续保留原有的过滤条件
                        !file.name.toLowerCase().includes('lib/') &&
                        !file.name.toLowerCase().includes('@') &&
                        !file.name.toLowerCase().includes('constructor-args.txt') &&
                        // 过滤掉其他非必要文件
                        !file.name.toLowerCase().includes('metadata.json') &&
                        !file.name.toLowerCase().includes('.txt') &&
                        !file.name.toLowerCase().includes('.json')
                      )
                      .map((file: any) => {
                        // 添加文件类型标记，帮助 AI 更好地理解内容
                        return `File: ${file.name} (Solidity Contract)\n${file.content}`;
                      })
                      .join('\n\n');

                    if (!content) {
                      throw new Error('No Solidity contract files found');
                    }
                  } else {
                    throw new Error('Invalid contract source code format');
                  }
                } catch (error: any) {
                  console.error('Error fetching from Telos API:', error);
                  throw new Error(`Failed to fetch contract source code: ${error.message}`);
                }
