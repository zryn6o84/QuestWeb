...
use crate::{
    error::QuestWebError,
    instruction::QuestWebInstruction,
...
impl Processor {
    pub fn process(
        ...
        let instruction = QuestWebInstruction::unpack(instruction_data)
            .map_err(|_| ProgramError::InvalidInstructionData)?;

        match instruction {
            QuestWebInstruction::InitializeBoard {
                name,
                description,
                img,
                config,
            ...
            QuestWebInstruction::CreateTask {
                name,
                description,
                deadline,
                max_completions,
            ...
            QuestWebInstruction::SelfCheckSubmission {
                signature,
                check_data,
            } => {
    ...
    fn process_initialize_board(
        ...
        if !rent.is_exempt(board_info.lamports(), board_info.data_len()) {
            return Err(QuestWebError::NotRentExempt.into());
        }
    ...
    fn process_create_task(
        ...
        if !board.is_initialized {
            return Err(QuestWebError::BoardNotInitialized.into());
        }
        if board.closed {
            return Err(QuestWebError::BoardIsClosed.into());
        }
    ...
    fn process_submit_proof(
        ...
        if member_pda != *member_info.key || member_info.data_borrow().is_empty() {
            return Err(QuestWebError::NotABoardMember.into());
        }

        // Verify task state
        let task = Task::try_from_slice(&task_info.data.borrow())?;
        if task.completed {
            return Err(QuestWebError::TaskAlreadyCompleted.into());
        }
        if task.cancelled {
            return Err(QuestWebError::TaskIsCancelled.into());
        }
        if task.deadline > 0 && task.deadline < Clock::get()?.unix_timestamp {
            return Err(QuestWebError::TaskDeadlinePassed.into());
        }
    ...
    fn process_review_submission(
        ...
        if !is_reviewer {
            return Err(QuestWebError::NotATaskReviewer.into());
        }
    ...
    fn process_self_check_submission(
        ...
        if !task.allow_self_check {
            return Err(QuestWebError::SelfCheckNotAllowed.into());
        }
