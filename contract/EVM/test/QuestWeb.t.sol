...
import {IQuestWeb} from "../src/IQuestWeb.sol";
...
contract QuestWebTest is Test {
    QuestWeb public QuestWeb;
    ...
    function setUp() public {
        ...
        QuestWeb = QuestWeb(payable(address(proxy)));
    }

    function testCreateQuestWeb() public {
        string memory name = "Test Board";
        string memory description = "This is a test board";
        string memory img = "http://example.com/image.png";

        QuestWeb.createQuestWeb(name, description, img, address(rewardToken));

        (uint256 id, address creator, string memory boardName, string memory boardDescription, , , , , ) = QuestWeb.boards(0);
    ...
    function testCreateTask() public {
        ...
        string memory img = "http://example.com/image.png";
        QuestWeb.createQuestWeb(boardName, boardDescription, img, address(rewardToken));
        ...
        QuestWeb.TaskView[] memory tasks = QuestWeb.getTasksForBoard(0);
    ...
    function testJoinBoard() public {
        ...
        QuestWeb.joinBoard(0);
    }

    function testUpdateQuestWeb() public {
        // First create a board
        string memory initialName = "Test Board";
        string memory initialDesc = "This is a test board";
        string memory img = "http://example.com/image.png";
        QuestWeb.createQuestWeb(initialName, initialDesc, img, address(rewardToken));
        ...
        QuestWeb.updateQuestWeb(0, newName, newDescription, newImg, newRewardToken);
    ...
    function testPledgeTokens() public {
        ...
        if (address(rewardToken) != address(0)) {
            MockERC20(address(rewardToken)).approve(address(QuestWeb), type(uint256).max);
        }

        uint256 pledgeAmount = 1000;
        // If it is a native token, the value needs to be sent.
        if (address(rewardToken) == address(0)) {
            QuestWeb.pledgeTokens{value: pledgeAmount}(0, pledgeAmount);
        } else {
            QuestWeb.pledgeTokens(0, pledgeAmount);
        }

        (,,,,,, uint256 totalPledged,,) = QuestWeb.boards(0);
    ...
    function testSubmitProof() public {
        ...
        string memory img = "http://example.com/image.png";
        QuestWeb.createQuestWeb(boardName, boardDesc, img, address(rewardToken));
        ...
        QuestWeb.submitProof(0, 0, proof);
        ...
        ) = QuestWeb.bountySubmissions(0, 0, address(this));
    ...
    function testReviewSubmission() public {
        ...
        rewardToken.approve(address(QuestWeb), 500);
        QuestWeb.pledgeTokens(0, 500);

        // Submit proof
        string memory submissionProof = "http://example.com/proof";
        QuestWeb.submitProof(0, 0, submissionProof);

        // Directly proceed to review (as the creator is already a reviewer)
        QuestWeb.reviewSubmission(0, 0, address(this), 1, "Good job!");
    ...
    function testSelfCheckSubmission() public {
        ...
        (
            ...
            uint256 submittedAt,
            string memory reviewComment
        ) = QuestWeb.bountySubmissions(0, 0, user);
    ...
    function testSetSignerAddress() public {
        ...
        QuestWeb.setSignerAddress(newSigner);
        assertEq(QuestWeb.signerAddress(), newSigner);
    }

    function testFailSetSignerAddressUnauthorized() public {
        address unauthorized = makeAddr("unauthorized");
        vm.prank(unauthorized);
        QuestWeb.setSignerAddress(unauthorized);
    }

    function testFailSelfCheckWithInvalidSignature() public {
        // Create sections and tasks
        string memory boardName = "Test Board";
        QuestWeb.createQuestWeb(boardName, "", "", address(rewardToken));
        QuestWeb.createTask(0, "Task", "", 0, 1, 100, "", true);
        ...
        vm.prank(user);
        QuestWeb.selfCheckSubmission(0, 0, signature, checkData);
    }
    ...
    function testUserTaskStatus() public {
        ...
        QuestWeb.submitProof(0, 0, proof1);

        // 5. Get section details and verify user task status.
        QuestWeb.BoardDetailView memory boardDetail = QuestWeb.getBoardDetail(0);
        ...
        QuestWeb.UserTaskStatus memory status1 = boardDetail.userTaskStatuses[0];
        ...
        QuestWeb.UserTaskStatus memory status2 = boardDetail.userTaskStatuses[1];
        ...
        rewardToken.approve(address(QuestWeb), 1000);
        QuestWeb.pledgeTokens(0, 1000);

        // 9. Test the status change after audit.
        QuestWeb.reviewSubmission(0, 0, user, 1, "Good job!");

        vm.prank(user);  // Switch back to the user context to view the results
        boardDetail = QuestWeb.getBoardDetail(0);
        ...
        QuestWeb.submitProof(0, 0, "New proof");
    }
    ...
    function testUserTaskStatusWithMultipleSubmissions() public {
        ...
        for(uint i = 0; i < users.length; i++) {
            vm.prank(users[i]);
            QuestWeb.submitProof(0, 0, string.concat("Proof from user ", Strings.toString(i)));

            // Get and verify the submission status of each user.
            vm.prank(users[i]);
            QuestWeb.BoardDetailView memory boardDetail = QuestWeb.getBoardDetail(0);
            QuestWeb.UserTaskStatus memory status = boardDetail.userTaskStatuses[0];
        ...
        for(uint i = 0; i < users.length; i++) {
            QuestWeb.reviewSubmission(0, 0, users[i], reviewStatuses[i], comments[i]);
    ...
    function testBoardViews() public {
        ...
        // 4. Submit task
        vm.prank(member);
        QuestWeb.submitProof(0, 0, "Test proof");

        // 5. Test getAllBoards
        console.log("Testing getAllBoards:");
        QuestWeb.BoardView[] memory allBoards = QuestWeb.getAllBoards();
        ...
        QuestWeb.BoardDetailView memory detail = QuestWeb.getBoardDetail(0);
        ...
        QuestWeb.BoardView[] memory memberBoards = QuestWeb.getBoardsByMember(member);
