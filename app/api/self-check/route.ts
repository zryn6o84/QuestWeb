...
import { lineaSepolia, Chain, mantleSepoliaTestnet, mantle, linea, flowMainnet, flowTestnet, bsc, bscTestnet, opBNB, opBNBTestnet, telosTestnet, telos } from 'viem/chains';
...
export async function POST(req: NextRequest) {
  try {
    const { questId, boardConfig, taskId, address, proof, chainName, task } = await req.json();

    // Validate parameters
    if (!questId || !taskId || !address || !proof || !chainName || !task) {
    ...
    const contractAddr = contractAddress.QuestWeb[chain.name as keyof typeof contractAddress.QuestWeb];
    if (!contractAddr) {
      return NextResponse.json(
    ...
    const message = encodeAbiParameters(
      ...
      [BigInt(questId), BigInt(taskId), address as `0x${string}`, checkData]
    );
